...
class ReviewViewTestCase(ItemViewTestCase):
    ...
    def setUp(self):
        ...
        # define other urls
        self.list_url = reverse("team:category:item:review:list",
            kwargs={
        ...
        self.create_url = reverse("team:category:item:review:create",
            kwargs={
                'team_slug': self.item.team.slug,
                'category_slug': self.item.category.slug,
                'item_slug': self.item.slug,
        ...
        self.update_url = reverse("team:category:item:review:update",
            kwargs={
                'team_slug': self.item.team.slug,
                'category_slug': self.item.category.slug,
                'item_slug': self.item.slug,
        ...
        self.delete_url = reverse("team:category:item:review:delete",
            kwargs={
                'team_slug': self.item.team.slug,
                'category_slug': self.item.category.slug,
                'item_slug': self.item.slug,
...
class ReviewListViewTest(ReviewViewTestCase):
    ...
    def test_review_list_member(self):
        ...
        for member in [self.team2_admin, self.team2_member, self.common_member]:
            ...
            item = self.item.category.items.exclude(
                pk=self.item.pk
            ).first()
...
class ReviewUpdateViewTest(ReviewViewTestCase):
    ...
    def test_review_update_regular_member(self):
        ...
        self.user = self.review.team.members.filter(
            memberships__admin=False
        ).exclude(
            pk=self.review.actor.pk
        ).order_by("?").first().user
...
class ReviewDeleteViewTest(ReviewViewTestCase):
    ...
    def test_review_delete_member(self):
        ...
        self.client.force_login(
            self.review.team.members.filter(
                memberships__admin=False
            ).exclude(
                pk=self.review.actor.uuid
            ).order_by("?").first().user
        )
        response = self.client.post(
            path=self.delete_url,
    ...
    def test_review_delete_admin(self):
        ...
        self.assertContains(
            response,
            "Reviews for %s" % self.review.item.name,
            status_code=200
        )
