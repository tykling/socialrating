...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        restaurant = Category.objects.create(
            team=foodteam, name="Restaurant", description="A place where food is served"
        )
        dish = Category.objects.create(
            team=foodteam, name="Dish", description="Some food"
        )
    ...
    def mock(self, model, factory, count, chunksize=10, bulk=True, **kwargs):
        ...
        """
        logger.info(
            "Creating %s instances of model %s using factory %s and kwargs %s"
            % (count, model, factory, kwargs)
        )
        if bulk:
            for i in range(0, count, chunksize):
                chunk_items = factory.build_batch(
                    count - i if count - i < chunksize else chunksize, **kwargs
                )
